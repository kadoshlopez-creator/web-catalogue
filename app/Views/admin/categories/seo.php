<div class="mb-6 flex items-center justify-between">
    <div class="flex items-center gap-4">
        ...
        <h2 class="text-2xl font-semibold text-gray-800">
            SEO: <?= htmlspecialchars($category['name']) ?>
        </h2>
    </div>
...
<form action="/admin/categories/<?= $category['id'] ?>/seo" method="POST" id="seoForm">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea id="meta_description" name="meta_description" rows="3"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors"
                            placeholder="Descubre nuestra amplia selección de laptops y computadoras portátiles al mejor precio..."><?= htmlspecialchars($category['meta_description'] ?? '') ?></textarea>
                    </div>
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="open_graph_title" value="<?= htmlspecialchars($category['open_graph_title'] ?? '') ?>" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm">
                        </div>
                        <div>
                            <label class="block text-sm text-gray-700 mb-1">OG Description</label>
                            <textarea name="open_graph_description" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"><?= htmlspecialchars($category['open_graph_description'] ?? '') ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="twitter_title" value="<?= htmlspecialchars($category['twitter_title'] ?? '') ?>" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm">
                        </div>
                        <div>
                            <label class="block text-sm text-gray-700 mb-1">Twitter Description</label>
                            <textarea name="twitter_description" rows="2" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"><?= htmlspecialchars($category['twitter_description'] ?? '') ?></textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <input type="url" name="canonical_url" value="<?= htmlspecialchars($category['canonical_url'] ?? '') ?>" placeholder="https://..." class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">JSON-LD Schema</label>
                        <textarea name="schema_json" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm font-mono text-xs" placeholder='{"@context": "https://schema.org","@type": "CollectionPage"}'><?= htmlspecialchars($category['schema_json'] ?? '') ?></textarea>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col gap-3">
                ...
                <a href="/admin/categories" class="w-full text-center text-gray-600 hover:text-gray-900 py-2 text-sm font-medium">
                    Cancelar
                </a>
            </div>
                    ...
                    <div class="font-sans">
                        <div class="text-[14px] text-[#202124] flex items-center gap-2 mb-1">
                            ...
                            <div>
                                ...
                                <div class="text-[12px] text-[#4d5156] leading-tight" id="serpUrl">https://misitio.com/categoria/<?= htmlspecialchars($category['slug'] ?? 'mi-categoria') ?></div>
                            </div>
                        </div>
                        <div class="text-[20px] text-[#1a0dab] hover:underline cursor-pointer mb-1 leading-snug" id="serpTitle">
                            <?= htmlspecialchars($category['meta_title'] ?: $category['name']) ?>
                        </div>
                        <div class="text-[14px] text-[#4d5156] leading-snug" id="serpDesc">
                            <?= htmlspecialchars($category['meta_description'] ?: 'Sin descripción SEO configurada. Google tomará contenido aleatorio de la página.') ?>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6 space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="robots_index" value="1" class="sr-only peer" <?= ($category['robots_index'] ?? 1) ? 'checked' : '' ?>>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" name="robots_follow" value="1" class="sr-only peer" <?= ($category['robots_follow'] ?? 1) ? 'checked' : '' ?>>
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        </label>
                    </div>
                    ...
                    <div class="pt-3 border-t border-gray-100">
                        ...
                        <input type="number" step="0.1" min="0" max="1" name="priority" value="<?= htmlspecialchars($category['priority'] ?? '0.5') ?>" class="w-full px-3 py-1.5 border border-gray-300 rounded text-sm">
                    </div>
                </div>
            </div>
