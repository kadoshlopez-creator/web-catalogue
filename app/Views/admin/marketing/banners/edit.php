<div class="mb-6">
    <div class="flex items-center text-sm text-gray-500 mb-2">
        ...
        <span class="text-gray-900 font-medium">Editar Banner</span>
    </div>
    <h2 class="text-2xl font-bold text-gray-800">Editar Banner</h2>
</div>
...
<form action="/admin/marketing/banners/<?= $banner['id'] ?>" method="POST" enctype="multipart/form-data" class="max-w-4xl">
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="title" name="title" value="<?= htmlspecialchars($banner['title'] ?? '') ?>"
            ...
            <div>
                ...
                <?php if(!empty($banner['image_path'])): ?>
                    <div class="mb-4 rounded-lg overflow-hidden border border-gray-200 relative group">
                        <img src="<?= htmlspecialchars($banner['image_path']) ?>" alt="Banner Actual" class="w-full h-auto object-cover max-h-64 block">
                        
                        <!-- Delete button (triggers external form) -->
                        <button type="button" onclick="if(confirm('¿Está seguro que desea eliminar el archivo?')) document.getElementById('delete-banner-form').submit();" class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-red-500 hover:bg-red-600 text-white rounded-full p-2 shadow-lg flex items-center justify-center cursor-pointer" title="Eliminar banner" style="z-index: 50;">
                    ...
                    <div class="space-y-1 text-center">
                        ...
                        <div class="mt-4 flex text-sm leading-6 text-gray-600 justify-center">
                            <label for="image" class="relative cursor-pointer rounded-md bg-white font-semibold text-blue-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-blue-600 focus-within:ring-offset-2 hover:text-blue-500">
                                ...
                                <input id="image" name="image" type="file" class="sr-only" accept="image/jpeg,image/png,image/webp" onchange="previewImage(event)">
                            </label>
                            <p class="pl-1">para reemplazar el actual</p>
                        </div>
                ...
                <!-- Image Preview -->
                <div id="image_preview_container" class="mt-4 hidden rounded-lg overflow-hidden border border-gray-200 border-dashed">
                    <img id="image_preview" src="" alt="Vista previa de reemplazo" class="w-full h-auto object-cover max-h-64 opacity-50">
                    <p class="text-center text-sm text-blue-600 font-bold py-2 bg-blue-50">Esta imagen reemplazará a la anterior al guardar</p>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 pt-6">
                <div>
                    <label for="link" class="block text-sm font-medium text-gray-700 mb-1">Enlace / URL de destino</label>
                    <input type="text" id="link" name="link" value="<?= htmlspecialchars($banner['link'] ?? '') ?>"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                </div>
                <div>
                    <label for="sort_order" class="block text-sm font-medium text-gray-700 mb-1">Orden de aparición</label>
                    <input type="number" id="sort_order" name="sort_order" value="<?= (int)$banner['sort_order'] ?>" min="0" 
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 pt-6">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" <?= $banner['is_active'] ? 'checked' : '' ?> class="w-5 h-5 text-blue-600 rounded focus:ring-blue-500">
                    <div>
                        <span class="block text-sm font-medium text-gray-900">Activar Banner</span>
                    </div>
                </label>
            </div>
...
<form id="delete-banner-form" action="/admin/marketing/banners/<?= $banner['id'] ?>/delete" method="POST" style="display: none;">
    <input type="hidden" name="_csrf_token" value="<?= \App\Core\Session::csrfToken() ?>">
</form>
