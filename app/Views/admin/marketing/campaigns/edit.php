<div class="mb-6">
    <div class="flex items-center text-sm text-gray-500 mb-2">
        ...
        <span class="text-gray-900 font-medium">Editar Campaña</span>
    </div>
    <h2 class="text-2xl font-bold text-gray-800">Editar Campaña: <?= htmlspecialchars($campaign['name']) ?></h2>
</div>

<form action="/admin/marketing/campaigns/<?= $campaign['id'] ?>" method="POST" class="max-w-3xl">
    <input type="hidden" name="_csrf_token" value="<?= \App\Core\Session::csrfToken() ?>">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
        <div class="p-6 border-b border-gray-100">
            <h3 class="text-lg font-semibold text-gray-800">Información General</h3>
            <p class="text-sm text-gray-500 mt-1">Modifica los detalles básicos del evento comercial.</p>
        </div>
        <div class="p-6 space-y-6">
            
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nombre de la Campaña *</label>
                <input type="text" id="name" name="name" required value="<?= htmlspecialchars($campaign['name']) ?>" 
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" 
                    class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"><?= htmlspecialchars($campaign['description'] ?? '') ?></textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="start_date" class="block text-sm font-medium text-gray-700 mb-1">Fecha de Inicio</label>
                    <input type="datetime-local" id="start_date" name="start_date" value="<?= $campaign['start_date'] ? date('Y-m-d\TH:i', strtotime($campaign['start_date'])) : '' ?>"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                </div>
                <div>
                    <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">Fecha de Fin (Opcional)</label>
                    <input type="datetime-local" id="end_date" name="end_date" value="<?= $campaign['end_date'] ? date('Y-m-d\TH:i', strtotime($campaign['end_date'])) : '' ?>"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                </div>
            </div>
            ...
            <div>
                ...
                <select id="status" name="status" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all bg-white">
                    <option value="draft" <?= $campaign['status'] == 'draft' ? 'selected' : '' ?>>Borrador</option>
                    <option value="scheduled" <?= $campaign['status'] == 'scheduled' ? 'selected' : '' ?>>Programada</option>
                    <option value="active" <?= $campaign['status'] == 'active' ? 'selected' : '' ?>>Activa</option>
                    <option value="ended" <?= $campaign['status'] == 'ended' ? 'selected' : '' ?>>Finalizada</option>
                </select>
            </div>
